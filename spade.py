...
def spade(device, use_cuda, arch, top_k, to_plot, to_dump):
    save_path = './spade/mvtec_result'
    data_path = 'C:/Users/Mikhail/Documents/Study/diploma/mvtec'

    # load model
    model, t_d, _ = load_model.load_model(arch)
    ...
    for class_name in mvtec.CLASS_NAMES:
        ...
        if arch not in load_model.ONELAYER_ARCHS:
            ...
            train_outputs = OrderedDict([('layer1', []), ('avgpool', [])])
            test_outputs = OrderedDict([('layer1', []), ('avgpool', [])])
        ...
        dist_matrix = utils.calc_dist_matrix(
            torch.flatten(test_outputs['avgpool'], 1),
        ...
        print('%s ROCAUC: %.3f' % (class_name, img_roc_auc))
        if to_plot:
            fig_img_rocauc.plot(fpr, tpr, label='%s ROCAUC: %.3f' % (class_name, img_roc_auc))

        score_map_list = []
        for t_idx in tqdm(range(test_outputs['avgpool'].shape[0]), '| localization | test | %s |' % class_name):
            score_maps = []
            for layer_name in layers:  # for each layer
        ...
        if to_plot:
            utils.visualize_loc_result(test_imgs, gt_mask_list, score_map_list, threshold, save_path, class_name, vis_num=5)
